...
__global__ void play(int *X, int *d_new, int N){
    ...
    for(int i = (blockIdx.x*blockDim.x)+threadIdx.x; i < N; i += blockDim.y*gridDim.y){
...
int main(int argc, char **argv){
    ...
    if(argc !=6){
        printf("Usage: %s filename size t threads blocks, where:\n", argv[0]);
        printf("\tfilename is the input file \n");
        printf("\tsize is the grid side and \n");
        printf("\tt generations to play\n");
        printf("\t threadsXthreads per block arranged in\n");
        printf("\t blocksXblocks\n");
        die("Wrong arguments");
    }
    ...
    //need next power of 2 to make sure all cells are taken into account
    int Npow2 = nearestPower(N);
    dim3 threadsPerBlock(thrds, thrds); //16x16 = 256 threads per block
    dim3 numBlocks(blks, blks); //split board into blocks
    //CUDA - timing
    float gputime;
...
void write_to_file(int *X, char *filename, int N){
    ...
    sprintf(newfilename, "table%dx%d_cuda.bin", N, N);
